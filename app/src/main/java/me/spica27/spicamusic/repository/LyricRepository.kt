...
class LyricRepository(
    ...
    suspend fun fetchLyric(
        title: String,
        artist: String?,
    ): ApiResponse<List<LyricResponse>> = lyricApi.fetchLyric(title, artist)

    suspend fun saveSongLyric(
        lyric: LyricResponse,
        song: Song,
    ...
    fun getLyrics(mediaId: Long) =
        lyricDao
            .getLyricsFlow(mediaId)
            .distinctUntilChanged()

    fun getDelay(mediaId: Long) =
        lyricDao
            .getDelayFlow(mediaId)
            .map {
                it ?: 0
            }.distinctUntilChanged()
