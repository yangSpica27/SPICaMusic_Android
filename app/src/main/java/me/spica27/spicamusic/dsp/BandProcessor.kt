...
class BandProcessor(
    ...
    private val beta =
        tan((band.bandwidth / 2.0) * PI / (sampleRate / 2.0)) * (
            sqrt(abs((GB.pow(2)) - (G0.pow(2)))) /
                sqrt(
        ...
        if (band.gain > 0) {
            ...
            if (band.bandwidthGain < referenceGain || band.gain < band.bandwidthGain) {
                throw IllegalArgumentException(
                    "Invalid parameters. Boost gain ($band.gain) must be greater than bandwidth gain ($band.bandwidthGain), which must be greater than reference ($referenceGain)",
                )
            }
        } else if (band.gain < 0) {
            // Cut
            if (band.bandwidthGain > referenceGain || band.gain > band.bandwidthGain) {
                throw IllegalArgumentException(
                    "Invalid parameters. Cut gain ($band.gain) must be less than bandwidth gain ($band.bandwidthGain), which must be less than reference ($referenceGain)",
                )
            }
        }
    }
