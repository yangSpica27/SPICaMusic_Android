    ...
    private fun extractAudioInfoWithTaglib(
        ...
        try {
            ...
            contentResolver.openFileDescriptor(uri, "r")?.use { pfd ->
                try {
                    // 使用 Taglib 读取音频属性
                    val audioProperties = TagLib.getAudioProperties(
                        fd = pfd.detachFd(),
                        readStyle = AudioPropertiesReadStyle.Fast
                    )

                    if (audioProperties != null) {
                        duration = audioProperties.length.toLong()
                        sampleRate = audioProperties.sampleRate
                        bitRate = audioProperties.bitrate
                        channels = audioProperties.channels

                        Timber.tag(TAG).v(
                            "Taglib 读取成功: duration=${duration}ms, " +
                                "sampleRate=${sampleRate}Hz, bitRate=${bitRate}kbps, channels=$channels"
                        )
                    }
